...
namespace DiagnoseTool
{
    public static class PdfReportService
    {
        ...
        private static byte[] GetCpmLogoPng(int width, int height)
        {
            ...
            // Top of C
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushTop, Data = Geometry.Parse("M16.68,11c.23-.45,.58-.84,1-1.12,.38-.3,.86-.46,1.34-.45l15.71,.17L37.36,.24,16.38,0c-1.68-.04-3.33,.39-4.77,1.26-1.46,.9-2.73,2.06-3.76,3.43-1.14,1.52-2.09,3.17-2.83,4.92-.82,1.85-1.53,3.74-2.14,5.66,.01,.06,.01,.11,0,.17l11.76,.14c.6-1.56,1.28-3.09,2.04-4.58Z") });
            // Bottom of C
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushBottom, Data = Geometry.Parse("M13.16,32.09c-.47-.12-.89-.38-1.21-.75-.32-.37-.46-.86-.38-1.34,.31-2.93,.9-5.81,1.75-8.63,.36-1.15,.76-2.27,1.19-3.38l-11.75-.11c-.58,1.87-1.07,3.73-1.46,5.56-.38,1.83-.74,3.56-1,5C.05,29.85-.05,31.29,.02,32.72c.06,1.44,.33,2.86,.81,4.21,.42,1.26,1.13,2.4,2.07,3.34,.98,.97,2.27,1.58,3.65,1.73,6.58,.88,13.25,.8,19.8-.24l2.67-9.76c-2.59,.53-5.23,.79-7.87,.78-2.66-.02-5.32-.26-7.94-.71l-.05,.02Z") });
            // Top of P
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushTop, Data = Geometry.Parse("M50.93,7.64l6.6,.07c.42,0,.83,.13,1.18,.36,.36,.24,.66,.54,.9,.9,.25,.38,.44,.79,.57,1.23,.13,.42,.2,.87,.2,1.31,0,1.19-.21,2.37-.63,3.49-.15,.4-.33,.79-.54,1.16l10.07,.12c0-.17,.12-.34,.17-.51,.55-1.98,.85-4.02,.88-6.08,.01-1.04-.15-2.07-.48-3.05-.31-.96-.8-1.86-1.44-2.64-1-1.16-2.27-2.04-3.71-2.56-1.41-.51-2.89-.78-4.38-.8l-18-.2-4.4,15.42,10.59,.14,2.42-8.36Z") });
            // Bottom of P
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushBottom, Data = Geometry.Parse("M59.2,18.53c-.33,.62-.72,1.21-1.18,1.75-.29,.34-.57,.67-.84,1-.29,.33-.63,.61-1,.84-.46,.26-.95,.45-1.46,.56-.72,.14-1.46,.2-2.19,.18l-5.31-.06,1.29-4.42-10.59-.12-6.76,23.59,10.59,.15,3.21-11.12,12.25,.12c1.03,.02,2.06-.19,3-.62,.95-.42,1.84-.96,2.65-1.62,.85-.69,1.62-1.48,2.3-2.34,.67-.86,1.29-1.76,1.86-2.7,.94-1.6,1.69-3.3,2.23-5.07l-10.05-.12Z") });
            // M left
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushBottom, Data = Geometry.Parse("M67.7,42.28l10.5,.12,6.7-23.57-10.5-.13-6.7,23.58Z") });
            // M center
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushBottom, Data = Geometry.Parse("M88.92,34h2.31l11.46-15-16.23-.19,2.46,15.19Z") });
            // M right
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushBottom, Data = Geometry.Parse("M104.15,19.05l-6.77,23.57,10.64,.13,6.68-23.58-10.55-.12Z") });
            // Top of M
            canvas.Children.Add(new System.Windows.Shapes.Path { Fill = brushTop, Data = Geometry.Parse("M119.11,1.33l-9.55-.11c-1.26,0-2.49,.31-3.59,.92-1.18,.65-2.2,1.54-3,2.63l-6.95,9.37-1.43-9c-.11-.6-.34-1.18-.68-1.69-.66-1-1.62-1.76-2.75-2.16-.55-.19-1.13-.29-1.71-.29l-10.64-.14-4.39,15.43,10.5,.12,1-3.53,.57,3.56,16.23,.19,2.32-3-.87,3.07,10.56,.12,4.38-15.49Z") });
